...
> jax bundle]
        ...
        whereis pandoc,"C:\Users\natorresf\Desktop\Pandoc\pandoc.exe"
...
* run R
        ...
        local cmd `" "`r(R)'" --vanilla < "`filename'.R" > "`filename'.rout" 2>
>  "`filename'.err" "'          
        shell `cmd'
        ...
        for(i = 1; i <= 6; i++) {
                ...
                while( (bot = ustrpos(line, tags[i], pos)) > 0) {       
                        ...
                        if (bot > 1 & bot + w < ustrlen(line)) {
                                ignore = usubstr(line, bot - 1, 1) == "`" & usu
> bstr(line, bot + w, 1) == "`"
                        }
                        if(!ignore) {
...
void fputvec(real scalar fh, string vector lines, | real scalar bot, real scala
> r top) {
